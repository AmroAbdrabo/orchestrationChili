...
CelluloBluetooth{
    ...
    property vector3d kGoalPoseErr: Qt.vector3d(2,2,2.3)    ///< Goal pose error coefficients when tracking pose/velocity, do not change if you don't have a good reason
    ...
    function setGoalPoseAndVelocity(x, y, theta, Vx, Vy, w){
        _private.goalPose = Qt.vector3d(x,y,theta);
        _private.goalVel = Qt.vector3d(Vx,Vy,w);
    ...
    onPoseChanged: {
        _private.estimateVel();
        if(poseVelControlEnabled)
            _private.commandVel();
    }

    onBootCompleted: _private.init()
    onPoseVelControlPeriodChanged: _private.init()
    onConnectionStatusChanged: _private.init()
    onPoseVelControlEnabledChanged: _private.init()

    onKidnappedChanged: _private.velEstimateNeedsReset = true
